...
def mkdir(wdir,new=False):
    if type(wdir) is list:
        ...
        if not os.path.exists(wdir):
            ...
            print('%s is not a directory!' %  wdir, sys.stderr)
...
def replace_between(string, repl, start_label=None, end_label=None):
    ...
    if i == -1:
        print("label %s is not found!" % start_label, sys.stderr)
        return ""
    if j == -1:
        print("label %s is not found!" % end_label, sys.stderr)
        return ""
...
def find_between(string, start_label=None, end_label=None):
    ...
    if i == -1:
        print("label %s is not found in %s!" % (start_label, string), sys.stderr)
        return ""
    if j == -1:
        print("label %s is not found in %s!" % (start_label, string), sys.stderr)
        return ""
...
def memory_check():
    ...
    mem = virtual_memory()
    #memory by GB
    N = int(mem.total) /1024/1024/1024  # total physical memory available
    return N
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def voidi():
