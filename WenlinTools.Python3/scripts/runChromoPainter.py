...
#main
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#many parameters inside this function
...
if __name__=='__main__':
    ...
    try:
        fn, fdonor, freceipt = list(map(os.path.abspath, sys.argv[1:4]))
    except:
        print("Usage: *.py fasta donorPopDef receiptIDs", file=sys.stderr)
        sys.exit()

    #so 10M should be 1 morgan
    wdir = '%s_%s_%s' % (cmn.lastName(fn), cmn.lastName(fdonor), cmn.lastName(freceipt))
    cmn.mkdir(wdir)
    os.chdir(wdir)
    outlabel = fasta2chpInput(fn, fdonor, freceipt)

    #run chromopainter
    #parameters = '-b  -i 15 -in -im -s 10 -ip -c'
    parameters = '-b  -i 15 -in -im -s 10 -ip'
    cmd = 'chromopainter -r %s.recomb -f %s.donor -g %s.hap %s > chp.log' % (outlabel, outlabel, outlabel, parameters)
    cmn.run(cmd)

    cmd = 'gunzip -f %s.hap.copyprobsperlocus.out.gz' % outlabel
    cmn.run(cmd)
