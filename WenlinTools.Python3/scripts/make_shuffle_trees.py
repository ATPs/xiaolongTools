...
if __name__=='__main__':
    ...
    try:
        ...
        print("Usage: *.py shuffle_genome shuffle", file=sys.stderr)
        sys.exit()

    fns = [os.path.abspath(i) for i in cmn.cmd2lines('ls %s/*' % indir)]

    wdir = 'shuffle_run_%s' % label
    cmn.mkdir(wdir)
    os.chdir(wdir)
    cmn.mkdir('job_files')
    for count, fn in enumerate(fns):
        ...
        dn = 'job_files/shuflle%s.job' % count
        cmn.run('make_job.py "%s" -p 256GB -t 48 > %s' % (cmd, dn))
