...
if __name__=='__main__':
    ...
    alea_list = cmn.cmd2lines('ssh butterfly@toxea.swmed.edu "ls /archive/butterfly/unbias_pipeline_info/step4_postprocessing/cleanMito/*/*.vcfCov"')
    ...
    for ID in IDs:
        taken = []
        for wdir in wdirs:
            cmd = 'ls %s/%s*.vcfCov  2> /dev/null| grep -v MITO | grep -v mitogenome %s' % (wdir, ID, required)
    ...
    falist.append('')
    cmn.write_lines(falist, 'cleanMitoStatList')
