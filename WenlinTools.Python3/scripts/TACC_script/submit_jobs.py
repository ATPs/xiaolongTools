...
def submit_job(dn, cores, pre, i, partition):
    ...
    cmd = ''
    incmd = '/work/00412/mtang/sequencing/scripts/master_run_jobs.py %s %s' % (dn, cores)
    cmd += '/work/00412/mtang/sequencing/scripts/make_job.py "%s" -p %s > %s/M%s_%s.job;' % (incmd, partition, tmpdir, pre, i)
    cmd += 'cd %s;' % tmpdir
    cmd += 'sbatch M%s_%s.job' % (pre, i)
    os.system(cmd)
