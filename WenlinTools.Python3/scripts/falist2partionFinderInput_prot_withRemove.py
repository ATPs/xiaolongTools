...
if __name__=='__main__':
    ...
    try:
        fnlist, fbad, outdir = sys.argv[1:]
    except:
        print("Usage: *.py falist toRemove.list outdirName", file=sys.stderr)
        sys.exit()

    badIDs = set(cmn.file2lines(fbad))
    ...
    new.append('format datatype=protein interleave=no gap=-;')
    new.append('matrix')
    for name in seqDict:
        new.append('%s        %s' % (name, seqDict[name]))
