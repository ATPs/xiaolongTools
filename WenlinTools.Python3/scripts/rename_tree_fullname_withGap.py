...
if __name__=='__main__':
    ...
    try:
        ...
        print("Usage: *.py RAxML_bestTree.noGap *gapCheck", file=sys.stderr)
        sys.exit()
    ...
    gapDict = {}
    for line in cmn.file2lines(fgap):
        items = line.strip().split()
        sp = items[0].split('_')[0]
        gap = '%.2f' % float(items[-1])
        gapDict[sp] = gap
    ...
    for node in t:
        ...
        new_name = '%s_gap%s_cp%s' % (fullname, gapDict[sp], appear[sp])
        if float(gapDict[sp]) > 0.5:
            new_name = '***' + new_name
