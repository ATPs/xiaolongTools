...
def tell_best_fa_quick(alist):
    ...
    if len(biohpc_files) != 0:
        bestOne = biohpc_files[-1]
    elif alea_files != []:
        alea_files = alea_files[-1:]
...
if __name__=='__main__':
    ...
    IDs = [parse_ID(each) for each in set(cmn.file2lines(fn))]
    ...
    wdirs = [
            ...
            '/archive/biophysics/Nick_lab/mtang/unbias_SNPs/*/step4_postprocessing/map2fasta'
            ]

    alea_list = cmn.cmd2lines('ssh butterfly@toxea.swmed.edu "ls /archive/butterfly/unbias_pipeline_info/step4_postprocessing/map2fasta/*.fa| grep -v mitogenome"')
    ...
    for ID in IDs:
        taken = []
        for wdir in wdirs:
            cmd = 'ls %s/%s_*_m2s.fa  2> /dev/null| grep -v MITO | grep -v mitogenom %s' % (wdir, ID, required)
            taken += cmn.cmd2lines(cmd)

        taken += [each for each in alea_list
                if cmn.lastName(each).split('_')[0] == ID and 'MITO' not in cmn.lastName(each)]
    ...
    for ID in faDict:
        ...
        if len(taken) == 0:
            ...
            falist.append(tell_best_fa_quick(taken))
