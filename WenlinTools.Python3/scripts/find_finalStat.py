...
if __name__=='__main__':
    ...
    wdirs = [
            './',
            '/project/biophysics/Nick_lab/mtang/archive/step4_postprocessing/final_stats',
            '/archive/biophysics/Nick_lab/mtang/unbias_SNPs/*/step4_postprocessing/final_stats',
            '/project/biophysics/Nick_lab/mtang/unbias_SNPs/*/step4_postprocessing/final_stats'
            ]

    alea_list = cmn.cmd2lines('ssh butterfly@toxea.swmed.edu "ls /archive/butterfly/unbias_pipeline_info/step4_postprocessing/final_stats/*.report"')
    ...
    for ID in IDs:
        taken = []
        for wdir in wdirs:
            cmd = 'ls %s/%s*.report  2> /dev/null| grep -v MITO | grep -v mitogenome %s' % (wdir, ID, required)
