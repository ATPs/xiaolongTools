...
def find_barcode_of_itself(ID, br_results):
    ...
    for line in br_results:
        ...
        checkList = sseqid.strip().split('|')
        #print sseqid
        if SRNP in checkList:
            found_SRNP = True
            print(SRNP, checkList)
...
if __name__=='__main__':
    ...
    for name in orderlist:
        ...
        ass_fasta = '>%s(assembled)\n%s\n' % (name, seqDict[name])
        #paired_fa.append(fasta)
        br_result = do_barcode_blast(ass_fasta)
        ...
        if selfBarcode == None:
            ...
            if len(difference) == 0:
                ass_fasta = '>=' + ass_fasta[1:]
            paired_fa.append(ass_fasta)
            ...
            bfa = '>%s\n%s\n' % (subjct, seq)
            difference = compare_itself(ass_fasta, bfa)
            if len(difference) == 0:
                ass_fasta = '>=' + ass_fasta[1:]
            paired_fa.append(ass_fasta)
            paired_fa.append(bfa)
