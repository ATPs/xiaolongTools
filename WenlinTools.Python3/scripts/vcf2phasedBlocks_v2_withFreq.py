...
def output_phase_position(phase1, phase2, base, mut):
    ...
    if '-1' in phase1:
        ...
        sys.exit()
    return char1, char2, isSwap, phase1.split('-')[0]
...
def phase_letter(items):
    ...
    if GT == './.':#no coverage
        ...
        isSwap = False #flag to see if we need to swap the copy counts
        if 'HP' in infoHeader_list:
            #phased
            isPhased = True
            phase1, phase2 = info_list[infoHeader_list.index('HP')].split(',')
            char1, char2, isSwap, curPhase = output_phase_position(phase1, phase2, base, mut)
            if isSwap:
                copy_counts.reverse()

        else:
            #if this position is not phasable, [Changed] still output the possible forms
        ...
        if 'HP' in infoHeader_list:
            isPhased = True
            phase1, phase2 = info_list[infoHeader_list.index('HP')].split(',')
            char1, char2, isSwap, curPhase = output_phase_position(phase1, phase2, char1, char2)
            if isSwap:
                copy_counts.reverse()
    ...
    if copy_counts != None:
        copy_counts = ','.join(map(str, copy_counts))
    return char1, char2, isHetero, isPhased, copy_counts
...
if __name__=='__main__':
    ...
    try:
        ...
        print("Usage: *.py phased.vcf", file=sys.stderr)
        print("difference from v0: label phasingInconsistent", file=sys.stderr)
        sys.exit()
    ...
    with open(fn) as fp:
        for line in fp:
            ...
            char1, char2, isHetero, isPhased, copyCounts = phase_letter(items)
            if isPhased:
                if 'PhasingInconsistent' in line:
                    phased_letter.append('%s\t%s\t%s\t%s\t%s\t(phase:%s, PhasingInconsistent)' % (scaf, index, char1, char2, copyCounts, curPhase))
                else:
                    phased_letter.append('%s\t%s\t%s\t%s\t%s\t(phase:%s)' % (scaf, index, char1, char2, copyCounts, curPhase))
            else:
                #not phased, but could be hetero positions
                if isHetero:
                    phased_letter.append('%s\t%s\t%s\t%s\t%s\t(noPhase)' % (scaf, index, char1, char2, copyCounts))
