...
if __name__=='__main__':
    ...
    faDict = {}
    for ID in list(IDs) + sum(list(alternatives.values()), []):
        taken = []
        for wdir in wdirs:
            cmd = 'ls %s/%s_*_m2s.fa  2> /dev/null| grep MITO %s' % (wdir, ID, required)
    ...
    for ID in IDs:
        ...
        if len(taken) == 0:
            try:
                otherIDs = alternatives[ID]
            except KeyError:
                otherIDs = []

            for otherID in otherIDs:
                taken += [each for each in faDict[otherID]
                        if best_ass in each.replace('_withMito', '')]
