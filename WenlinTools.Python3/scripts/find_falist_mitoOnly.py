...
def count_ass_appearance(alist):
    ...
    for fn in alist:
        ...
        ass = '_'.join(items[1:-2]).replace('_withMito', '').replace('_mitogenome', '').replace('_withNCBImito', '').replace('_NCBImitogenome', '').replace('_with_mito', '')
        if ass == 'cne':
...
if __name__=='__main__':
    ...
    wdirs = [
            '/project/biophysics/Nick_lab/mtang/archive/step4_postprocessing/cleanMito/*',
            '/project/biophysics/Nick_lab/mtang/unbias_SNPs/*/step4_postprocessing/cleanMito/*',
            '/archive/biophysics/Nick_lab/mtang/unbias_SNPs/*/step4_postprocessing/cleanMito/*',
            './step4_postprocessing/cleanMito/*'
            ]

    #alea_list = cmn.cmd2lines('ssh butterfly@toxea.swmed.edu "ls /archive/butterfly/unbias_pipeline_info/step4_postprocessing/cleanMito/*/*MITOonly*.fa"')
    alea_list = cmn.cmd2lines('ssh butterfly@alea.swmed.edu "ls /archive/butterfly/unbias_pipeline_info/step4_postprocessing/cleanMito/*/*MITOonly*.fa"')
    ...
    for ID in IDs:
        taken = []
        for wdir in wdirs:
            cmd = 'ls %s/%s_*MITOonly*.fa  2> /dev/null %s' % (wdir, ID, required)
    ...
    if len(IDs) != len(falist):
        print('Error! falist file has different number of lines compared to NickList')
        print('please ask Wenlin for help')
    ...
    if len(missing) != 0:
        ...
        for ID in missing:
            try:
                label = ref_mito_dict[ID]
            except KeyError:
                tmp.append(ID)
                continue
            if label in best_ass:
                falist.append('/work/biophysics/mtang/SNP_calling/indexed_references/mitogenomes/%s_mito.fa' % ID)
            else:
                tmp.append(ID)
        missing = tmp
    ...
    if len(missing) != 0:
        ...
        print('\n'.join(missing))
        cmn.write_lines(missing, 'missingMITOonlys')
    else:
        cmd = 'rm missingMITOonlys 2> /dev/null'
        cmn.run(cmd)

    falist.append('')
    cmn.write_lines(falist, 'falist.mitoOnly')
