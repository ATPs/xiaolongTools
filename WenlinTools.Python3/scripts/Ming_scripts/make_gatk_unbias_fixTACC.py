...
def make_bwa_cmds(fastqs, reflabel, wdir):
    ...
    for key in paired:
        ...
        cmd += '/home2/wli/local/bwa-0.7.12/bwa mem -t 48 -M %s %s %s > %s;\n' % (reflabel, lib1, lib2, fsam)
        fsams.append(fsam)

    cmd += '/home2/wli/local/bwa-0.7.12/bwa mem -t 48 -M %s %s/%s > %s/single.sam;\n' % (reflabel, outdir, cmn.lastName(unpaired), outdir)
...
if __name__=='__main__':
    ...
    try:
        ...
        print("Usage: *.py ../step1_gather_data/mapping_info.txt ../step2_bwa_mapping ../step1_gather_data/require_SNPs.dict.pkl", file=sys.stderr)
        sys.exit()
    ...
    template = cmn.txt_read('/work/biophysics/mtang/SNP_calling/scripts/templates/template_gatk_unbias.job')
    ...
    for sp in refdict:
        ...
        for samdir, ref in snp_list:
            ...
            fass = '%s/%s.fa' % (refdir, ref)
            #wdir = '%s/%s_%s' % (cwd, sp, ref)
            olabel = '%s_%s' % (sp, ref)
            if olabel not in badones:
                continue
            wdir = '%s/%s' % (cwd, olabel)
            cmn.mkdir(wdir)
    ...
    #3. preparing the directories
        ...
        #merge sam files into one
            cmd = 'python /work/biophysics/mtang/SNP_calling/scripts/merge_mapped_sams.py %s/%s.sam %s' % (wdir, olabel, ' '.join(fsams))
            cmds.append(cmd)
    ...
    #the new reference is named 'assembly_selfref_v2.fa'
            cmd, fnewSams = make_bwa_cmds(fq_dict[sp], 'assembly_selfref_v2', wdir)
            info = info.replace('[WL_mapping_cmds]', cmd)
    #7. merge mapped sams
            cmd = 'python /work/biophysics/mtang/SNP_calling/scripts/merge_mapped_sams.py %s_step2.sam %s' % (olabel, ' '.join(fnewSams))
