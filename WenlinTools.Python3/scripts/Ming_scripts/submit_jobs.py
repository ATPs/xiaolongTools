...
def submit_job(dn, cores, pre, i, partition):
    ...
    incmd = '/work/biophysics/mtang/SNP_calling/scripts/master_run_jobs.py %s %s' % (dn, cores)
    cmd += '/work/biophysics/mtang/SNP_calling/scripts/make_job.py "%s" -p %s > %s/M%s_%s.job;' % (incmd, partition, tmpdir, pre, i)
    cmd += 'cd %s;' % tmpdir
