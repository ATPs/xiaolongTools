...
def submit_job(dn, Npara, pre, i, partition):
    tmpdir, dn2 = dn.split('/')
    cmd = 'source /home2/wli/.bash_profile;'
    incmd = 'master_run_jobs_each.py %s %s' % (dn, Npara)
...
if __name__ == '__main__':
    try:
        ...
        N = int(sys.argv[2])
        Npara = int(sys.argv[3])
    except:
        print('usage: *.py cmd_file split_parts NumberOfParalelCmds [prefix4spilt,def:x] [-p super] [-t 24]', file=sys.stderr)
        sys.exit()
