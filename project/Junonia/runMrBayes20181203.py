...
def runMrBayes(file_in, folder_bayes = None, folder_newick = None, folder_temp = None):
    ...
    # filter fasta seqs # remove a seq if the gapratio is over max_gapratio
    file_filter = os.path.join(folder_temp,basename+'.filter')
    fasta.alignmentFilterGapSeqs(seqs_in=file_in, seqs_out=file_filter, max_gapratio = 0.7)
    
    # convert fasta to nexus format
    file_nex = os.path.join(folder_temp,basename)
    fasta2nexus.changeAlignment2Nex(file_in=file_filter, file_out=file_nex)
