...
def read_range(fn):
    ...
    with open(fn) as fp:
        for line in fp:
            ...
            name = items[0].split('.e')[0]
            i, j = map(int, items[1:3])
...
def computeConservedLetterDNAwithCloseGroupExclusion(fa, fn, fclose, fbad, fout, fexclude, frange, frename, gapInGroup=1.0, gapInClose=0.9, gapOutGroup=0.7, consOutGroup=0.7, outfilename = None):
    ...
    for gene in rangeDict:
        i, j = rangeDict[gene]
        print (gene, i, j, j-i)
        seglist = [(name, seq[i:j]) for name, seq in seqlist]
        segOther = [(name, seq[i:j]) for name, seq in otherlist]
        segClose = [(name, seq[i:j]) for name, seq in closeSeqList]
    ...
    char_info = sorted(allInfo, key=lambda x: x[-3])
...
if __name__ == '__main__':
    ...
    f = parser.parse_args()
    computeConservedLetterDNAwithCloseGroupExclusion(fa=f.fa, fn=f.fn, fclose=f.fclose, fbad=f.fbad, fout=f.fout, fexclude=f.fexclude, frange=f.frange, frename=f.frename, gapInGroup=f.gapInGroup, gapInClose=f.gapInClose, gapOutGroup=f.gapOutGroup, consOutGroup=f.consOutGroup, outfilename=f.outfilename)
