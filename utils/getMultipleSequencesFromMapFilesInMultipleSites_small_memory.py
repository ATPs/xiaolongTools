...
MAPLINES = 0 #store the number of lines in map file
...
def extractSequenceFromMapFileInSites(filename,strand = 0):
    ...
    for fragment_ID, fragment_sites in sites:
        sBaseInt = mapBaseInt[fragment_sites]
        seq = ''.join(chr(e) for e in sBaseInt)
...
def mapfilelineprocessing(x):
    x1,x2 = x.split(':')
    sites = StringIO(x2)
    x2 = np.loadtxt(sites,dtype=int) # use np.int array to save memory
    return x1,x2

def mapfileGetSites2FileInMemory(file_sites):
    '''
    get sites from file_sites. Use funtion to release memory more efficiently
    '''
...
def getSequencesFromMapFilesInSites(files_map,file_sites,outfolder,strand = 0, threads = 16):
    ...
    global MAPLINES
    MAPLINES = 0# get the number of lines in map file
    for e in open(files[0]):
        MAPLINES += 1
    print('map file line number is:',MAPLINES)
    
    mapfileGetSites2FileInMemory(file_sites)
...
output multiple files to the output folder. sequence names is processed as:
    filename of map file is like '5518_Junonia_coenia_JC_v1.0.scaffolds_snp_step2.map' or '/archive/butterfly/maps/debiased/5518_Junonia_coenia_JC_v1.0.scaffolds_snp_step2.map'
    return '5518' as the sequence name
    if 'snp_step2' in (filename) do that. otherwise, return basename with no .map. '/home/xcao/w/20180905Junonia_coenia/20181010Trees/20181026MergeSample/J_c_coenia.map' return J_c_coenia
