...
def multipleFastaHaploid2STRUCTUREinputOneThread(df_nt, gapcut = 0.8, Ncut=4, missing_data = -9):
    ...
    df_snp = pd.DataFrame(rows_keep)
    return df_snp

def multipleFastaHaploid2STRUCTUREinput(filename,outfile = 'default', gapcut = 0.8, Ncut=4, missing_data = -9, threads = 32):
    '''
    ...
    df_snp.to_csv(outfile,header = False,sep='\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-N','--Ncut',help = 'minumum number of counts to consider as a SNP, default 4. (>=4)',default = 4)
    parser.add_argument('-M','--missing_data',help = 'set value other than ATCG to this value, default -9',default = -9)
    f = parser.parse_args()
    multipleFastaHaploid2STRUCTUREinput(filename = f.input,outfile = f.output, gapcut = f.gapcut, Ncut=f.Ncut, missing_data = f.missing_data, threads=f.threads)
