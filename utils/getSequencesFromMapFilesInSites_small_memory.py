...
def extractSequenceFromMapFileInSites(filename,strand = 0):
    ...
    sBaseInt = mapBaseInt[sites]
    return ''.join(chr(e) for e in sBaseInt)
...
def getSequencesFromMapFilesInSites(files_map,file_sites,outfile=None,strand = 0, threads = 16):
    ...
    if type(file_sites) is not str:
        sites = file_sites
    else:
        sites = open(file_sites).read().split()
        sites = np.array([int(e) for e in sites])
...
description = '''
input a file with filepath map files and sites to keep, output a fasta file. This program can also be imported in other modules to return the value directly
Keep the base order in sites file
It will be twice slower than the normal one, but it can save a lot of memories
'''
